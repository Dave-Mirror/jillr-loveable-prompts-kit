...
const Leaderboard = () => {
  ...
  return (
    <PageContainer previousPage="/explore" nextPage="/wallet">
      <div className="container mx-auto max-w-[1440px] pb-20">
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          {/* Decorative elements */}
          <motion.div 
            className="absolute -top-6 left-4 md:left-12 text-jillr-neonPurple/10 transform rotate-12 scale-125"
          ...
          <motion.div 
            className="absolute -top-4 right-4 md:right-16 text-jillr-neonGreen/10 transform -rotate-12 scale-125"
          ...
          <motion.div 
            className="absolute top-16 left-1/2 -translate-x-1/2 text-jillr-neonPink/10 scale-150"
            animate={{ scale: [1.4, 1.5, 1.4], opacity: [0.1, 0.15, 0.1] }}
            transition={{ repeat: Infinity, duration: 3 }}
          >
            <Crown size={72} strokeWidth={1.5} />
          </motion.div>

          {/* Gradient overlay */}
          <div className="absolute inset-0 bg-gradient-to-b from-background to-transparent pointer-events-none h-52"></div>
